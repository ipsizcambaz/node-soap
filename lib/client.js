...
Client.prototype.addSoapHeader = function(soapHeader, name, namespace, xmlns) {
    ...
    if (typeof soapHeader === 'object') {
        soapHeader = this.wsdl.objectToXML(
            soapHeader,
            name,
            namespace,
            xmlns,
...
Client.prototype.changeSoapHeader = function(
    index,
    soapHeader,
    name,
    namespace,
...
Client.prototype.addBodyAttribute = function(
    bodyAttribute,
    name,
    namespace,
    xmlns
    ...
    if (typeof bodyAttribute === 'object') {
        var composition = '';
        Object.getOwnPropertyNames(bodyAttribute).forEach(function(
            prop,
            idx,
            array
        ) {
...
Client.prototype._initializeOptions = function(options) {
    ...
    this.wsdl.options.forceHTTPS = options.forceHTTPS || false;
...
Client.prototype._defineService = function(service, endpoint) {
    ...
    for (var name in ports) {
        const ep = endpoint ? endpoint : ports[name].location;
        def[name] = this._definePort(
            ports[name],
            this.wsdl.options.forceHTTPS ? ep.replace('http:', 'https:') : ep
        );
    }
    return def;
};
...
Client.prototype._invoke = function(
    ...
    if (
        this.streamAllowed &&
        typeof self.httpClient.requestStream === 'function'
    ) {
        callback = _.once(callback);
        var startTime = Date.now();
        req = self.httpClient.requestStream(
            location,
            xml,
            headers,
            options,
        ...
        req.on('response', function(response) {
            ...
            if (
                ...
                response.pipe(
                    concatStream({ encoding: 'string' }, function(body) {
    ...
    if (req && req.headers)
        //fixes an issue when req or req.headers is indefined
        self.lastRequestHeaders = req.headers;
};
